...
const OUTPUT_DIR = `${CSII_USERDATAPATH}\\Mods\\BelzontWE\\UI`;
...
module.exports = (en) => {
  const outDir = en.production ? "dist" : OUTPUT_DIR
  return {
    mode: "production",
    stats: "none",
    entry: {
    ...
    module: {
      rules: [
        ...
        {
          test: /\.s?css$/,
          include: [path.join(__dirname, "src"), path.join(__dirname, "..", "_shared")],
          use: [
