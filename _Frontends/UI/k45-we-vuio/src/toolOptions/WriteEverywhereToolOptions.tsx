...
type State = {
    displayAppearenceWindow?: boolean
}
...
class WEWorldPickerToolPanel extends Component<{}, State> {
    ...
    constructor(props: any) {
        super(props);
        this.state = {}
        WorldPickerService.instance.registerBindings(this)
    }

    componentWillUnmount(): void {
        WorldPickerService.instance.disposeBindings()
    }
            ...
            <>
                ...
                {wps.CurrentItemIdx.value < wps.CurrentItemCount.value &&
                    <>
                        ...
                        <AmountValueSection
                            widthContent={120}
                            valueGetter={() => translateUnitResult([MetricUnitsEntries.distance.linear[UnitSystem.Metric][0], { VALUE: precisions[wps.MouseSensibility.value] + "" }]) + ` | ${(precisions[wps.MouseSensibility.value] * 10).toFixed(2)}°`}
                        ...
                        <VectorSectionEditable title={L_rotation}
                            ...
                            valueGetterFormatted={() => wps.CurrentRotation.value?.map(x => x.toFixed(3) + "°")}
                            onValueChanged={(i, x) => {
                        ...
                        <VectorSectionEditable title={L_scale}
                            ...
                            valueGetterFormatted={() => wps.CurrentScale.value?.map(x => x.toFixed(3))}
                            onValueChanged={(i, x) => {
                ...
                <VanillaComponentResolver.instance.Section title={L_actions}>
                    <>
                        {wps.CurrentItemIdx.value < wps.CurrentItemCount.value && <>
                            <VanillaComponentResolver.instance.ToolButton onSelect={() => this.setState({ displayAppearenceWindow: !this.state.displayAppearenceWindow })} selected={this.state.displayAppearenceWindow} src={i_AppearenceBtnIcon} focusKey={VanillaComponentResolver.instance.FOCUS_DISABLED} className={VanillaComponentResolver.instance.toolButtonTheme.button} tooltip={T_AppearenceBtn} />
                            <div style={{ width: "10rem" }}></div>
                        </>
                        }
                ...
                {wps.CurrentItemIdx.value < wps.CurrentItemCount.value && <>
                    {this.state.displayAppearenceWindow && <WETextAppearenceSettings />}
                </>}
            </>
    }
}
