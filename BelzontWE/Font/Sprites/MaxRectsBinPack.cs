...
public class MaxRectsBinPack : ISerializable
{
    ...
    public void Serialize<TWriter>(TWriter writer) where TWriter : IWriter
    {
        ...
        writer.Write(binHeight);
        writer.Write(allowRotations);

        writer.Write(usedRectangles.Count);
        for (int i = 0; i < usedRectangles.Count; i++)
        {
            writer.Write(usedRectangles[i].position);
            writer.Write(usedRectangles[i].size);
        }
        writer.Write(freeRectangles.Count);
        for (int i = 0; i < freeRectangles.Count; i++)
        {
            writer.Write(freeRectangles[i].position);
            writer.Write(freeRectangles[i].size);
        }
    }
    ...
    public void Deserialize<TReader>(TReader reader) where TReader : IReader
    {
        ...
        reader.Read(out binWidth);
        reader.Read(out binHeight);
        reader.Read(out allowRotations);
        ...
        for (int i = 0; i < Count; i++)
        {
            ...
            usedRectangles.Add(new(position, size));
        }
        ...
        for (int i = 0; i < Count; i++)
        {
            ...
            freeRectangles.Add(new(position, size));
        }
    }
}
