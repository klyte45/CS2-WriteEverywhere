                ...
                for (var i = 0; i < entities.Length; i++)
                {
                    ...
                    switch (weMeshData.TextType)
                    {
                        ...
                        case WESimulationTextType.GameProp:
                            // Swap WEWaitingRendering → WEWaitingGamePropRendering so WEGamePropSpawnSystem
                            // can consume it at Modification2B without conflicting with text/image rendering.
                            m_CommandBuffer.AddComponent<WEWaitingGamePropRendering>(unfilteredChunkIndex, entity);
                            m_CommandBuffer.SetComponentEnabled<WEWaitingGamePropRendering>(unfilteredChunkIndex, entity, true);
                            m_CommandBuffer.SetComponentEnabled<WEWaitingRendering>(unfilteredChunkIndex, entity, false);
                            break;
                        default:
                            if (m_WeIsPlaceholderLkp.HasComponent(entity)) m_CommandBuffer.SetComponentEnabled<WEIsPlaceholder>(unfilteredChunkIndex, entity, false);
